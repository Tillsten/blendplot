...
import cppyy
cppyy.add_include_path(r'C:\Users\tills\OneDrive\Dokumente\blend2d-beta12-all\blend2d\src')
...
clist = [
    ...
    ("BLMatrix2D", ["mapPoint", "mapVector"], stdov),
    ("BLImage", [], stdov),
    ("BLPoint", [], stdov),
    ("BLPointI", [], stdov),
    ...
    ("BLRgba32", [], stdov),
    ("BLPattern", [], stdov),  
    ("BLPath", ['getClosestVertex'], stdov),
    ("BLRect", [], stdov),
    ("BLFont", ['getGlyphOutlines', 'getGlyphRunOutlines', 'mapTextToGlyphs'], stdov),
    ("BLFontFace", [], stdov),
    ("BLCircle", [], stdov),
    ("BLGlyphRun", [], []),
    ("BLGlyphInfo", [], []),
                ...
                if len(decl) > 1:
                    for l in decl:                                       
                        ...
                        if m in const_fcn:
                            ...
                            extra = ''
                        w(2, '.def("%s", py::overload_cast<%s> (&%s::%s%s))'%(m, ga(args), klass, m, extra))
                else:                    
